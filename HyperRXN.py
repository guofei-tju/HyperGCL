...
class HyperRXN_v4(nn.Module):
    def __init__(self, input_num, hyper_num, input_feature_num, emb_dim, conv_dim, head=3, p=0.1, L=1,
                 use_attention=True, similarity=None):
        super(HyperRXN_v4, self).__init__()
        self.emb_dim = emb_dim
        self.conv_dim = conv_dim
        self.p = p
        self.input_num = input_num
        ...
        self.similarity = similarity
        self.relu = nn.ReLU()
        self.hypergraph_conv = hnn.HypergraphConv(2 * emb_dim, conv_dim, heads=head, use_attention=use_attention,
                                                  dropout=p)
        if L > 1:
            self.hypergraph_conv_list = []
            for l in range(L - 1):
        ...
        if use_attention:
            self.hyperedge_attr = nn.Parameter(torch.randn((hyper_num, 2 * emb_dim), dtype=torch.float),
                                               requires_grad=True)
            if L > 1:
                self.hyperedge_attr_list = []
                for l in range(L - 1):
                    self.hyperedge_attr_list.append(
                        nn.Parameter(torch.randn((hyper_num, head * conv_dim), dtype=torch.float),
                                     requires_grad=True))
        self.hyperedge_linear = nn.Linear(conv_dim * head, 2)
        self.softmax = nn.Softmax(dim=1)

    def forward(self, input_fetures, incidence_matrix):
        input_x = self.relu(self.linear_encoder(input_fetures))
        simi_feature = self.relu(self.similarity_liner(self.similarity))
        input_nodes_features = torch.cat((simi_feature, input_x), dim=1)
        row, col = torch.where(incidence_matrix.T)
        edges = torch.cat((col.view(1, -1), row.view(1, -1)), dim=0)
        input_nodes_features = self.hypergraph_conv(input_nodes_features, edges, hyperedge_attr=self.hyperedge_attr)
        if self.hyper_conv_L > 1:
            for l in range(self.hyper_conv_L - 1):
                input_nodes_features = self.hypergraph_conv_list[l](input_nodes_features, edges,
                                                                    hyperedge_attr=self.hyperedge_attr_list[l])
                input_nodes_features = self.relu(input_nodes_features)

        hyperedge_feature = torch.mm(incidence_matrix.T, input_nodes_features)
        return self.hyperedge_linear(hyperedge_feature)

    def get_rxn_embedding(self, input_fetures, incidence_matrix):
        input_x = self.relu(self.linear_encoder(input_fetures))
        simi_feature = self.relu(self.similarity_liner(self.similarity))
        input_nodes_features = torch.cat((simi_feature, input_x), dim=1)
        row, col = torch.where(incidence_matrix.T)
        ...
        if self.hyper_conv_L > 1:
            for l in range(self.hyper_conv_L - 1):
                ...
                input_nodes_features = self.relu(input_nodes_features)
        hyperedge_feature = torch.mm(incidence_matrix.T, input_nodes_features)
        return hyperedge_feature, self.softmax(self.hyperedge_linear(hyperedge_feature))
